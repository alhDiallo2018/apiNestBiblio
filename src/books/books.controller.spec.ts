...
describe('BooksController', () => {
  ...
  it('should call create method from service when createBook is called', async () => {
    ...
    // Appeler la méthode create du contrôleur
    const result = await controller.create(createBookDto);

    // Vérifier que le résultat correspond à la valeur renvoyée par le mock
    expect(result).toEqual({ title: 'NestJS Book', author: 'John Doe' });

    // Vérifier que la méthode create a été appelée avec le bon argument
